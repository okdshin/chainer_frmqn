...
class MemoryModule(Chain):
    ...
    def read(self, h):
        ...
        p = F.softmax(F.reshape(F.batch_matmul(M_key, h, transa=False, transb=False), (h.shape[0], M_key.shape[1]))) # (B, M)
        ...
        o = F.batch_matmul(p, M_val, transa=True, transb=False) # (B, 1, m)
